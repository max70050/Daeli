...
// --- NEU: Logik für Speisekarten-Verwaltung (Admin) ---

const adminMenuContainer = document.getElementById('admin-menu-container');
let currentMenuStatus = {}; // Cache für den Ausverkauft-Status

// Ruft den Status aller Artikel aus Firestore ab
...
// Rendert die Speisekarte in den Admin-Tools
async function renderAdminMenu() {
    ...
    adminMenuContainer.innerHTML = ''; // Leeren
    ...
    for (const id in products) {
        ...
        // Die Button-Klasse und der Text werden jetzt basierend auf dem Status bestimmt
...
// Schaltet den "Ausverkauft"-Status eines Artikels um
async function toggleSoldOutStatus(itemId) {
    ...
    try {
        ...
        // UI im Admin-Bereich sofort aktualisieren
        ...
        // Klassen und Text dynamisch umschalten
        if (newStatus) {
        ...
        // UI auf der Bestellseite aktualisieren
...
// Event-Listener für die Admin-Speisekarte
if (adminMenuContainer) {
    adminMenuContainer.addEventListener('click', e => {
        // Reagiert jetzt auf beide möglichen Button-Klassen
...
// Aktualisiert die Kundenansicht der Speisekarte basierend auf `currentMenuStatus`
...
const showOrderPage = async () => {
    ...
    if (user && user.emailVerified) {
        ...
        // NEU: Menüstatus laden und Ansicht aktualisieren
        await fetchMenuStatus();
        updateCustomerMenuView();
...
const showAdminToolsPage = () => {
    if (auth.currentUser) {
        ...
        renderAdminStats();
        renderAdminMenu(); // NEU HINZUGEFÜGT
    } else {
        openModal();
    }
};
...
const renderManagementOrders = async (day) => {
    ...
    orders.forEach(order => {
        ...
        if (order.isPaid) {
            ...
            const paymentStatusBadge = document.createElement('span');
            paymentStatusBadge.className = 'payment-status-badge paid';
            paymentStatusBadge.textContent = 'Bezahlt'; // Vereinheitlicht von "Online Bezahlt"
            headerInfo.appendChild(paymentStatusBadge);
        }
...
async function createOrderObject(paymentMethod, orderCart, orderPickupDay, orderPickupTime) {
    ...
    return {
        ...
        status: 'pending',
        paymentMethod,
        isPaid: true, // Hinzugefügt: Da diese Funktion nur für Guthaben-Zahlung genutzt wird, ist die Bestellung bezahlt.
...
async function saveOrderToFirestore(paymentMethod, orderCart, orderPickupDay, orderPickupTime) {
    ...
    try {
        ...
        const orderData = {
            ...
            status: 'pending',
            paymentMethod,
            isPaid: paymentMethod === 'Online-Zahlung', // Hinzugefügt: Nur 'true', wenn online bezahlt wurde.
            adminCompleted: false,
            userAcknowledged: false,
            isReported: false,
            isPrepared: false
